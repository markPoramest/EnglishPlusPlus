...
public class AdvanceMode extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db = appDB.getInstance(AdvanceMode.this);
        Thread t1 = new Thread(new Runnable() {
            @Override
            public void run() {
                vocab = (ArrayList<com.example.english.Model.vocab>) db.vocabDAO().getAlladvVocab();
                next(0, vocab);
                Log.e("db",vocab.size()+"");
    ...
    public final void dialogTrue(int score2, ArrayList db) {
       ...
       final AlertDialog.Builder alert = new AlertDialog.Builder(AdvanceMode.this);
        if (db.size() == 5) {
            ...
            alert.setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    Intent i1 = new Intent(AdvanceMode.this,result.class);
                    i1.putExtra("score",score1);
                    i1.putExtra("mode","Adv");
                    startActivity(i1);
                    finish();
                }
            });
